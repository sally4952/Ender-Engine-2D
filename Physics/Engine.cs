...
namespace EnderEngine2D.Physics
{
    ...
    internal class Engine
    {
        ...
        /// <summary>
        /// 通过引力方向初始化物理引擎（gravititionDirection会自动转换为单位向量并且除以这个单位向量得到重力值Gravitition）。
        /// </summary>
        /// <param name="gravititionDirection"></param>
                ...
                Task.Run(() =>
                {
                    ...
                    else if (intersectingBodies.Count > 0)
                    {
                        ...
                        Next:
                            ...
                            rigid.Force /= new Vector2(rigid.InertiaAttenuation, rigid.InertiaAttenuation);
                            rigid.Force += new Vector2(GravititionDirection.X * Gravitition, -GravititionDirection.Y * Gravitition);
                            if (MathUtils.InTolerance(rigid.Force, Vector2.Zero, 0.00002f))
                                rigid.Force = Vector2.Zero;
                        });
        ...
        private Dictionary<int, bool> mOldRigidBodiesStates;
        /// <summary>
        /// 暂停物理模拟（不需要停止调用Update方法）。
        /// </summary>
        public void Pause()
        {
            mOldRigidBodiesStates = new Dictionary<int, bool>();
            foreach (var rb in RigidBodies)
            {
                mOldRigidBodiesStates.Add(rb.Key, rb.Value.IsPhysicsal);
                rb.Value.IsPhysicsal = false;
            }
        }
        /// <summary>
        /// 继续物理模拟（不需要再次开始调用Update方法）。
        /// </summary>
        public void Continue()
        {
            foreach (var rb in RigidBodies)
            {
                if (mOldRigidBodiesStates.TryGetValue(rb.Key, out var value))
                {
                    rb.Value.IsPhysicsal = value;
                }
                else
                {
                    throw new Exception("I don't know also what fucking things happened. 可能是宇宙射线改变了你电脑中的内存。");
                }
            }
        }
